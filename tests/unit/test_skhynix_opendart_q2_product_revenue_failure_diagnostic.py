...
def _archive() -> bytes:
    ...
    <html><body>
      ...
      <table>
        <tr><th></th><th colspan="2">DRAM</th><th colspan="2">NAND Flash</th><th colspan="2">기타</th><th colspan="2">부문 합계</th></tr>
        <tr><th>구분</th><th>3개월</th><th>누적</th><th>3개월</th><th>누적</th><th>3개월</th><th>누적</th><th>3개월</th><th>누적</th></tr>
        <tr><td>수익(매출액)</td><td>730</td><td>1400</td><td>260</td><td>500</td><td>10</td><td>20</td><td>1000</td><td>1920</td></tr>
      </table>
    </body></html>
    """
    buffer = io.BytesIO()
...
def test_diagnostic_reports_relevant_text_and_table_shape(tmp_path: Path) -> None:
    ...
    raw = _archive()
    text = "\n".join([
        "연결재무제표 주석", "5. 매출액", "당반기", "(단위 : 백만원)",
        "DRAM", "3개월", "누적", "NAND Flash", "3개월", "누적",
        "기타", "3개월", "누적", "부문 합계", "3개월", "누적",
        "수익(매출액)", "730", "1400", "260", "500", "10", "20", "1000", "1920",
    ])
    ...
    assert table["rows"] == 3
    assert table["contains"]["dram"] is True
    assert table["contains"]["nand"] is True
