...
def test_q2_source_certification_launcher_uses_project_python_and_stays_offline() -> None:
    script = Path("scripts/report_skhynix_official_ir_q2_source_certification.ps1").read_text(
        encoding="utf-8"
    )
    ...
    assert "publication date must come from PDF bytes" in script
