...
ACTUALS = {
    "000660": {
        "revenue": (32_765_700_000_000.0, 66_193_000_000_000.0, 92_100_000_000_000.0),
        "operating_income": (-7_730_300_000_000.0, 23_467_000_000_000.0, 38_200_000_000_000.0),
        "net_income": (-9_112_400_000_000.0, 19_797_000_000_000.0, 31_500_000_000_000.0),
    },
    "005930": {
        "revenue": (258_935_500_000_000.0, 300_870_900_000_000.0, 333_600_000_000_000.0),
        "operating_income": (6_567_000_000_000.0, 32_726_000_000_000.0, 44_100_000_000_000.0),
        "net_income": (14_473_400_000_000.0, 34_451_000_000_000.0, 38_700_000_000_000.0),
    },
}


def _data_row(values: tuple[float, float, float], *, forecast_a: float, forecast_b: float) -> dict[str, str]:
...
def _write_expectation(root: Path, *, shuffle_revenue: bool = False) -> Path:
    ...
    for symbol in SYMBOLS:
        revenue = _data_row(ACTUALS[symbol]["revenue"], forecast_a=4_000_001, forecast_b=4_000_002)
        if shuffle_revenue:
        ...
        raw[symbol] = {
            ...
            "output2": output2,
            "output3": [_random_row(10_000.0 + index * 1_000.0) for index in range(output3_count)],
            ...
            rows.append(
                {
                    ...
                    "business_year": year,
                    "period_label": "FY",
                    "revenue": ACTUALS[symbol]["revenue"][index],
                    "operating_income": ACTUALS[symbol]["operating_income"][index],
                    "net_income": ACTUALS[symbol]["net_income"][index],
                }
            )
    import pandas as pd
...
def test_crosscheck_finds_three_unique_historical_actual_rows(tmp_path: Path) -> None:
    ...
    artifact = json.loads(Path(str(pointer["crosscheck_path"])).read_text(encoding="utf-8"))
    ...
    for metric in ("revenue", "operating_income", "net_income"):
        ...
        assert metrics[metric]["forecast_period_labels"] == ["2026.12E", "2027.12E"]
    ...
    assert (output_root / "latest_kis_expectation_semantic_crosscheck.json").read_bytes().isascii()
