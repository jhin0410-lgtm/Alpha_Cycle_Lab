...
def _dates() -> list[str]:
    return [
        *(f"202608{day:02d}" for day in range(7, 0, -1)),
        *(f"202607{day:02d}" for day in range(31, 18, -1)),
    ]
...
def _write_fixture(tmp_path: Path) -> Path:
    ...
    for ticker, price in (("005930", 100_000), ("000660", 200_000)):
        for index, row_date in enumerate(_dates()):
            ...
            rows.append(
                {
                    "ticker": ticker,
                    "date": row_date,
                    "current_price": str(-(price - index * 1_000)),
        ...
        json.dumps(
            {
                ...
                "captured_at": "2026-08-08T20:25:30+09:00",
...
def test_build_report_verifies_live_mapping_and_keeps_scoring_disabled(
    tmp_path: Path,
) -> None:
    ...
    assert report.provider_semantic_status == "provider_field_mapping_pending_live_certification"
    assert report.field_mapping_verified is True
    assert report.point_in_time_verified is True
    assert report.evidence_verified is True
    assert report.decision_score_enabled is False
    assert len(report.tickers) == 2
    diag = next(row for row in report.tickers if row.ticker == "005930")
    ...
    assert diag.rows_on_or_before_evaluation_date == 20

    five = report.window("005930", 5)
...
def test_request_contract_fails_closed_on_wrong_unit(tmp_path: Path) -> None:
    ...
    assert report.field_mapping_verified is False
    assert report.evidence_verified is False
    assert report.decision_score_enabled is False


def test_one_share_market_balance_error_fails_mapping_verification(tmp_path: Path) -> None:
    pointer = _write_fixture(tmp_path)
    pointer_payload = json.loads(pointer.read_text(encoding="ascii"))
    csv_path = Path(pointer_payload["investor_flows_path"])
    with csv_path.open("r", encoding="utf-8-sig", newline="") as handle:
        rows = list(csv.DictReader(handle))
    rows[0]["other_corporation_net_buy_shares"] = str(
        int(rows[0]["other_corporation_net_buy_shares"]) + 1
    )
    with csv_path.open("w", encoding="utf-8-sig", newline="") as handle:
    ...
    assert report.field_mapping_verified is False
    assert report.reason == "market_balance_mismatch:005930"
    assert report.evidence_verified is False


def test_captured_date_mismatch_fails_point_in_time_verification(tmp_path: Path) -> None:
    pointer = _write_fixture(tmp_path)
    pointer_payload = json.loads(pointer.read_text(encoding="ascii"))
    manifest_path = Path(pointer_payload["manifest_path"])
    manifest = json.loads(manifest_path.read_text(encoding="utf-8"))
    manifest["captured_at"] = "2026-08-09T09:00:00+09:00"
    manifest_path.write_text(json.dumps(manifest), encoding="utf-8")
    ...
    assert report.reason == "captured_date_mismatch"
    assert report.evidence_verified is False
