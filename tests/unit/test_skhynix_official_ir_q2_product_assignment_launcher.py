...
def test_product_assignment_launcher_uses_project_python_and_stays_offline() -> None:
    script = Path(
    ...
    command_index = script.index(
        "& $ProjectPython -m "
        "alpha_cycle.sk_hynix_official_ir_q2_product_assignment_certification_cli"
    )
    ...
    assert "numeric Other share and allocation remain disabled" in script
