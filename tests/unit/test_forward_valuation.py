        ...
        [
            {
                ...
                "annual_revenue": 1.0,
                "pe": 100_000_000_000.0 if complete is True else None,
                "ps": 100_000_000_000.0 if complete is True else None,
            }
        ]
    )
...
def test_forward_pe_uses_certified_net_income_and_pit_market_cap() -> None:
    ...
    assert state.guardrail_evidence_id == load_decision_system_v21_guardrails().evidence_id
...
def test_missing_forward_metric_never_falls_back_to_trailing_actual() -> None:
    ...
    assert state.payload_without_id()["trailing_actual_substitution_enabled"] is False
...
def test_market_cap_completeness_requires_real_boolean() -> None:
    with pytest.raises(ValueError, match="market_cap_complete must be a boolean"):
        build_forward_valuation_state(
            _valuation(complete="false"),
            _expectation_state((_expectation(),)),
        )
...
def test_multiple_certified_providers_remain_separate_instead_of_silent_average() -> None:
    ...
    assert state.payload_without_id()["provider_aggregation_enabled"] is False
...
def test_persistence_binds_both_sources_and_v21_guardrail(tmp_path: Path) -> None:
    state = build_forward_valuation_state(
        _valuation(),
        _expectation_state((_expectation(),)),
    )
    ...
    assert manifest["guardrail_evidence_id"] == state.guardrail_evidence_id
    assert manifest["available_multiple_count"] == 1
    assert manifest["trailing_actual_substitution_enabled"] is False
    assert manifest["provider_aggregation_enabled"] is False
