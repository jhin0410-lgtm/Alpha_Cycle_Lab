...
def test_intelligence_export_uses_scenario_final_decision_builder() -> None:
    assert (
        intelligence.build_investment_decision_snapshot.__module__
        == "alpha_cycle.intelligence.decision_scenario_expected_return_calibrated"
    )
    wrapper = Path(
        "src/alpha_cycle/intelligence/decision_scenario_expected_return_calibrated.py"
    ).read_text(encoding="utf-8")
    assert "decision_catalyst_horizon_calibrated" in wrapper
    assert "_build_catalyst_snapshot" in wrapper
