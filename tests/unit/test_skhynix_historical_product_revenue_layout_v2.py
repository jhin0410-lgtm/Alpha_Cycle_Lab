...
def _spec(year: int, quarter: int) -> PeriodicProductRevenueSpec:
    ...
    return PeriodicProductRevenueSpec(
        ...
        discovery_begin_date=date(year, month + 2 if month <= 9 else 12, 1),
        discovery_end_date=date(year, month + 2 if month <= 9 else 12, 28),
...
<table>
<tr><th>(단위: 백만원)</th><th>(단위: 백만원)</th><th>(단위: 백만원)</th></tr>
...
<table>
<tr><th>(단위: 백만원)</th><th>(단위: 백만원)</th><th>(단위: 백만원)</th><th>(단위: 백만원)</th><th>(단위: 백만원)</th></tr>
...
def test_q1_split_column_family_scopes_only_connected_current_quarter() -> None:
    spec = _spec(2025, 1)
    text = "\n".join(
        [
            "21. 매출액 (연결)",
    ...
    html = """<html><body>
<p>21. 매출액 (연결)</p><p>공시금액</p><p>당분기</p><p>(단위 : 백만원)</p>
...
def test_2024_q2_current_column_parser_accepts_products_and_services_total_alias() -> None:
    spec = _spec(2024, 2)
    text = "\n".join(
        [
            "21. 매출액 (연결)",
