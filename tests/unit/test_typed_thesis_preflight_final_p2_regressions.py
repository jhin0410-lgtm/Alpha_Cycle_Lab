...
def test_blocker_clear_appends_ready_transition_and_clears_observatory_state(tmp_path) -> None:
    ...
    assert blocked.ready_for_package_assembly is False
    ...
    assert ready.changed_history is True
    assert ready.ready_for_package_assembly is True
    assert ready.run is not None
    assert ready.run.kind is ResearchRunKind.PRE_ORCHESTRATION_READY
    assert ready.run.blockers == ()
    assert ready.run.research_round_snapshot_id is None
    assert ready.ledger.summary.run_count == 2
    assert ready.ledger.summary.blocked_run_count == 1
    assert dict(ready.ledger.summary.status_counts) == {
        "pre_orchestration_blocked": 1,
        "pre_orchestration_ready": 1,
    }
...
def test_replay_cutoff_a_b_a_reuses_prior_equivalent_preflight_event(tmp_path) -> None:
    receipt = _record_request(
        tmp_path,
    ...
    assert second_b.changed_history is True
    assert revisit_a.changed_history is False
    assert revisit_a.run == first_a.run
    assert revisit_a.ledger.snapshot_id == second_b.ledger.snapshot_id
    assert revisit_a.ledger.summary.run_count == 2
    assert revisit_a.ledger.summary.blocked_run_count == 2
    assert dict(revisit_a.ledger.summary.blocker_code_counts) == {
        "investment_thesis_snapshot_missing": 2
    }
...
def test_multi_security_preflight_scans_each_thesis_artifact_once(tmp_path, monkeypatch) -> None:
    ...
    assert preflight.run is not None
    assert preflight.run.kind is ResearchRunKind.PRE_ORCHESTRATION_READY
