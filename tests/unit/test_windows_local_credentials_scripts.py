...
def test_runner_loads_user_credentials_and_never_reads_missing_report_path() -> None:
    ...
    assert "$ProjectPython -m alpha_cycle.live_pipeline_provenance_cli" in script
    assert "$ProjectPython -m alpha_cycle.live_pipeline_cli" not in script
...
def test_cmd_launcher_bypasses_local_execution_policy_for_repo_script() -> None:
    ...
    orchestrator = _read("scripts/run_live_pipeline_orchestrator.ps1")
