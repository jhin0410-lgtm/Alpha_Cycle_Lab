...
처리 순서는 `MarketDataFeed → Strategy → TargetPosition → Order → RiskManager →
SimulatedBroker → Fill → Portfolio → Reporting`입니다. 전략 인터페이스에는 브로커가
전달되지 않으므로 직접 주문할 수 없습니다.

`MarketDataFeed.history_through(date)`는 현재 이벤트 날짜 이후 행을 반환하지 않습니다.
`next_open` 모드의 목표는 큐에 보관되었다가 다음 피드 이벤트의 시가로 주문화됩니다.
마지막 날 목표는 실행 가격이 없으므로 체결되지 않습니다. `same_close`는 전략이 당일
