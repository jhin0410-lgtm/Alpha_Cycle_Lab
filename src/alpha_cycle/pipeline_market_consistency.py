...
from alpha_cycle.market_consistency_assessment_integrity import assess_consistency_result
...
def _write_failed_scope_pointer(
    *,
    ...
    payload: dict[str, object] = {
        ...
        "raw_result_path": None if raw_result_path is None else str(raw_result_path),
    ...
    try:
        raw_integrity._atomic_json(
            output_root / "latest_market_scope_assessment.json", payload
        )
    except OSError:
        return
...
def _write_raw_and_scope_failure(
    *, output_root: Path, failure: BaseException, checked_at: datetime
) -> None:
    raw_integrity._write_failed_pointer(output_root, failure, checked_at)
    _write_failed_scope_pointer(
        output_root=output_root, failure=failure, checked_at=checked_at
    )


def _explicit_evidence(
    *, output_root: Path, market_directory: Path
    ...
    *,
    ...
    try:
        evidence = _explicit_evidence(
            output_root=root, market_directory=Path(market_directory)
        )
        staging_parent = root / ".market-consistency-staging"
        staging_parent.mkdir(parents=True, exist_ok=True)
        with tempfile.TemporaryDirectory(
            prefix="pipeline-", dir=staging_parent
        ) as staging_text:
        ...
        _write_raw_and_scope_failure(
            output_root=root, failure=failure, checked_at=checked_at
        )
        raise failure from exc
    except (core.ConsistencyError, OSError, TypeError, ValueError) as exc:
        _write_raw_and_scope_failure(
            output_root=root, failure=exc, checked_at=checked_at
        )
        raise

    try:
        assessment, assessment_path = assess_consistency_result(
            raw_result, raw_result_path, output_root=root
        )
...
__all__ = ["PipelineMarketConsistencyGate", "run_pipeline_market_consistency_gate"]
