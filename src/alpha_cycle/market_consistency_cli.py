...
This command never fetches data and never substitutes one provider for another.
It compares completed, unadjusted daily OHLC evidence and compares live quotes
only when the two capture times are sufficiently close.
"""
...
class SnapshotEvidence:
    ...
    prices: dict[str, tuple[Decimal, datetime | None]]
    candles: dict[tuple[str, date], tuple[Decimal, Decimal, Decimal, Decimal, Decimal]]


class ConsistencyError(ValueError):
    """Expected fail-closed evidence validation error."""
...
def _require_fields(row: Mapping[str, object], fields: Iterable[str], *, source: str) -> None:
...
def _snapshot_candidates(root: Path) -> list[tuple[datetime, Path]]:
    ...
    for manifest_path in root.glob("*/manifest.json"):
        ...
        try:
            ...
            captured = _parse_datetime(manifest.get("captured_at"), field="captured_at")
        except ConsistencyError:
            continue
        candidates.append((captured, manifest_path.parent))
...
def _resolve_toss_directory(output_root: Path) -> tuple[Path, str]:
    ...
    if latest_run_path.is_file():
        ...
        candidate_text = str(latest.get("market_directory", "")).strip()
        if latest.get("status") == "completed" and candidate_text:
            candidate = Path(candidate_text)
            if (candidate / "manifest.json").is_file():
                return candidate, "latest_run"
...
def _resolve_kiwoom_directory(output_root: Path) -> Path:
    ...
    pointer_snapshot = str(pointer.get("snapshot_id", "")).strip()
    manifest = _load_json(directory / "manifest.json")
    if pointer_snapshot != str(manifest.get("snapshot_id", "")).strip():
        raise ConsistencyError("Kiwoom latest pointer and manifest snapshot IDs differ")
    return directory


def _validate_symbol_set(manifest: Mapping[str, object], *, provider: str) -> tuple[str, ...]:
...
def _load_toss(directory: Path) -> SnapshotEvidence:
    ...
    if _boolean(manifest.get("order_api_enabled"), field="Toss order_api_enabled"):
        raise ConsistencyError("Toss snapshot unexpectedly enables order API")
    _validate_symbol_set(manifest, provider="TossInvest")
    snapshot_id = str(manifest.get("snapshot_id", "")).strip()
    if not snapshot_id:
        raise ConsistencyError("Toss manifest has no snapshot_id")
    captured_at = _parse_datetime(manifest.get("captured_at"), field="Toss captured_at")

    prices: dict[str, tuple[Decimal, datetime | None]] = {}
    for row in _read_csv(directory / "prices.csv"):
        _require_fields(row, ("symbol", "timestamp", "last_price", "currency"), source="prices.csv")
        symbol = row["symbol"].strip()
        if row["currency"].strip().upper() != "KRW":
            raise ConsistencyError(f"Toss currency is not KRW for {symbol}")
        timestamp = _parse_datetime(row["timestamp"], field=f"Toss price timestamp {symbol}")
        prices[symbol] = (
            _decimal(row["last_price"], field=f"Toss last_price {symbol}"),
            timestamp,
        )
    ...
    for row in _read_csv(directory / "candles.csv"):
        _require_fields(
            row,
            ("symbol", "timestamp", "open", "high", "low", "close", "volume", "currency", "interval", "adjusted"),
            source="candles.csv",
        )
        symbol = row["symbol"].strip()
        if row["currency"].strip().upper() != "KRW" or row["interval"].strip() != "1d":
            raise ConsistencyError(f"Toss candle basis mismatch for {symbol}")
        if _boolean(row["adjusted"], field=f"Toss candle adjusted {symbol}"):
            raise ConsistencyError(f"Toss candle is adjusted for {symbol}")
        timestamp = _parse_datetime(row["timestamp"], field=f"Toss candle timestamp {symbol}")
        candle_date = timestamp.astimezone(KOREA_TZ).date()
        key = (symbol, candle_date)
        if key in candles:
            raise ConsistencyError(f"duplicate Toss candle for {symbol} {candle_date}")
        candles[key] = (
...
def _load_kiwoom(directory: Path) -> SnapshotEvidence:
    manifest = _load_json(directory / "manifest.json")
    if manifest.get("status") != "completed" or manifest.get("provider") != KIWOOM_PROVIDER:
        raise ConsistencyError("Kiwoom manifest is not a completed OpenAPI+ export")
    if _boolean(manifest.get("adjusted_prices"), field="Kiwoom adjusted_prices"):
    ...
    _validate_symbol_set(manifest, provider="Kiwoom")
    snapshot_id = str(manifest.get("snapshot_id", "")).strip()
    if not snapshot_id:
    ...
    for row in _read_csv(directory / "quotes.csv"):
        ...
        prices[ticker] = (
            _decimal(row["current_price"], field=f"Kiwoom current_price {ticker}"),
            None,
        )
    ...
    for row in _read_csv(directory / "daily_bars.csv"):
        _require_fields(
            row,
            ("ticker", "date", "open_price", "high_price", "low_price", "close_price", "volume", "adjusted"),
            source="daily_bars.csv",
        )
        ticker = row["ticker"].strip()
        ...
        if key in candles:
            raise ConsistencyError(f"duplicate Kiwoom daily bar for {ticker} {candle_date}")
        candles[key] = (
...
def _compare_daily(
    ...
    *,
    ...
    for ticker in EXPECTED_SYMBOLS:
        toss_dates = {value for symbol, value in toss.candles if symbol == ticker and value < cutoff}
        kiwoom_dates = {value for symbol, value in kiwoom.candles if symbol == ticker and value < cutoff}
        overlap = sorted(toss_dates & kiwoom_dates, reverse=True)
        if len(overlap) < required_days:
            failures.append(
                f"{ticker}: only {len(overlap)} completed overlapping daily bars; require {required_days}"
            )
            continue
        symbol_conflicts = 0
        for candle_date in overlap[:required_days]:
            ...
            volume_bps = _bps_difference(toss_values[4], kiwoom_values[4])
            comparisons.append(
                DailyComparison(
                    ticker=ticker,
                    ...
                    volume_difference_bps=(
                        None if not volume_bps.is_finite() else str(volume_bps.quantize(Decimal("0.0001")))
                    ),
                )
            )
            if not price_match:
        ...
        if symbol_conflicts:
            failures.append(
                f"{ticker}: {symbol_conflicts} completed daily OHLC rows exceed price tolerance"
            )
        else:
            passed.append(ticker)
        volume_mismatches = sum(
            1 for item in comparisons if item.ticker == ticker and not item.volume_match
        )
        if volume_mismatches:
            warnings.append(
                f"{ticker}: {volume_mismatches} historical volume rows differ; OHLC remains authoritative for this gate"
            )
    return comparisons, tuple(passed), failures, warnings
...
def _compare_quotes(
    ...
    *,
    ...
    gap = abs(toss.captured_at.astimezone(UTC) - kiwoom.captured_at.astimezone(UTC))
    warnings: list[str] = []
    failures: list[str] = []
    comparable = (
        timedelta(0) <= toss_age <= max_snapshot_age
        and timedelta(0) <= kiwoom_age <= max_snapshot_age
        and gap <= max_capture_gap
    )
    if not comparable:
        reasons: list[str] = []
        ...
        rows = [
            QuoteComparison(
                ticker=ticker,
                toss_price=str(toss.prices[ticker][0]),
                kiwoom_price=str(kiwoom.prices[ticker][0]),
                absolute_difference_won=str(abs(toss.prices[ticker][0] - kiwoom.prices[ticker][0])),
                difference_bps=str(_bps_difference(toss.prices[ticker][0], kiwoom.prices[ticker][0])),
                capture_gap_seconds=str(gap.total_seconds()),
                comparable=False,
    ...
    conflict_count = 0
    for ticker in EXPECTED_SYMBOLS:
        toss_price = toss.prices[ticker][0]
        kiwoom_price = kiwoom.prices[ticker][0]
        difference = _bps_difference(toss_price, kiwoom_price)
        within = difference <= tolerance_bps
        if not within:
            conflict_count += 1
            failures.append(
                f"{ticker}: live quote difference {difference.quantize(Decimal('0.01'))} bps exceeds {tolerance_bps} bps"
            )
        rows.append(
            QuoteComparison(
                ticker=ticker,
                toss_price=str(toss_price),
                kiwoom_price=str(kiwoom_price),
                absolute_difference_won=str(abs(toss_price - kiwoom_price)),
                difference_bps=str(difference.quantize(Decimal("0.0001"))),
                capture_gap_seconds=str(gap.total_seconds()),
                comparable=True,
    ...
    return rows, "conflict" if conflict_count else "passed", failures, warnings
...
def run_consistency_check(
    *,
    ...
    daily_rows, historical_passed, daily_failures, daily_warnings = _compare_daily(
        toss,
        kiwoom,
    ...
    daily_conflicts = sum(not item.price_match for item in daily_rows)
    volume_mismatches = sum(not item.volume_match for item in daily_rows)
    comparable_quotes = sum(item.comparable for item in quote_rows)
    live_conflicts = sum(item.within_tolerance is False for item in quote_rows)
    ...
    status = "failed" if failures else (
        "passed" if integration_eligible else "passed_historical_only"
    )
    cutoff = min(
        toss.captured_at.astimezone(KOREA_TZ).date(),
        kiwoom.captured_at.astimezone(KOREA_TZ).date(),
    ...
    payload_without_id: dict[str, object] = {
        ...
        "toss_snapshot_age_seconds": (checked_at - toss.captured_at.astimezone(UTC)).total_seconds(),
        ...
        "kiwoom_snapshot_age_seconds": (checked_at - kiwoom.captured_at.astimezone(UTC)).total_seconds(),
    ...
    result_id = _result_id(payload_without_id)
    ...
    _atomic_json(output_root / "latest_market_consistency.json", latest)
    return result, result_path


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Cross-check immutable TossInvest and Kiwoom market evidence without fetching or substituting data"
    )
    parser.add_argument("--output-root", type=Path, default=DEFAULT_OUTPUT_ROOT)
    parser.add_argument("--required-days", type=int, default=20)
    parser.add_argument("--price-tolerance-won", type=Decimal, default=Decimal(0))
    parser.add_argument("--live-tolerance-bps", type=Decimal, default=Decimal(50))
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.json:
        ...
        print(f"historical price conflicts: {result.historical_price_conflict_count}")
        print(f"historical volume mismatches: {result.historical_volume_mismatch_count}")
        print(f"live quote status: {result.live_quote_status}")
        print(f"decision integration eligible: {result.decision_integration_eligible}")
