...
from .intelligence.sk_hynix_product_profitability_reduced_identifiable_fit import (
    build_reduced_identifiable_fit,
    build_reduced_prefit_identification,
    load_reduced_identifiable_rows,
)
from .intelligence.sk_hynix_product_profitability_reduced_identifiable_method import (
...
DEFAULT_REDUCED_IDENTIFIABLE_FIT_OUTPUT = (
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Fit frozen SK hynix V4 five-parameter reduced-identifiable model on the "
            "clean 21-row panel. 2026Q1 is contaminated stress only and 2026Q3 is sealed."
        )
    )
    parser.add_argument("--evaluation-date", required=True, type=date.fromisoformat)
    parser.add_argument("--method", default=str(DEFAULT_REDUCED_IDENTIFIABLE_METHOD))
    ...
    parser.add_argument("--v3-fit-report", default=str(DEFAULT_V3_FIT_REPORT))
    parser.add_argument("--output", default=str(DEFAULT_REDUCED_IDENTIFIABLE_FIT_OUTPUT))
    return parser
...
def _v3_comparison(path: Path, v4_loocv_mae: float) -> dict[str, object] | None:
    if not path.exists():
        return None
    raw: object = json.loads(path.read_text(encoding="utf-8"))
    if not isinstance(raw, dict):
        raise ValueError("V3 comparison report must be an object")
    root = {str(key): value for key, value in cast(dict[object, object], raw).items()}
    result = root.get("result")
    if not isinstance(result, dict):
        raise ValueError("V3 comparison report lacks result object")
    item = {str(key): value for key, value in cast(dict[object, object], result).items()}
    v3_mae = float(str(item.get("loocv_mae_krw_million", "nan")))
    return {
        "v3_method_version": "3.0-frozen-pre-fit",
        "v3_loocv_mae_krw_million": v3_mae,
        "v4_loocv_mae_krw_million": v4_loocv_mae,
        "v4_minus_v3_loocv_mae_krw_million": v4_loocv_mae - v3_mae,
        "used_for_v4_gate": False,
        "report_only": True,
    }
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    method = load_frozen_reduced_identifiable_method(Path(args.method))
    ...
    rows, contaminated_q1 = load_reduced_identifiable_rows(
        method,
        closeout,
        frontier,
    ...
    if not prefit.prefit_gate_passed:
        payload: dict[str, object] = {
            "schema_version": 1,
            "status": "skhynix_product_profitability_v4_prefit_gate_failed",
            "method_version": method.method_version,
            "method_evidence_id": method.evidence_id,
            "prefit_identification": asdict(prefit),
            "fit_attempted": False,
            "other_margin_claimed_zero": False,
            "other_contribution_role": "unmodeled_company_gross_profit_residual",
    ...
    comparison = _v3_comparison(Path(args.v3_fit_report), result.loocv_mae_krw_million)
    payload = {
        "schema_version": 1,
        "status": "skhynix_product_profitability_v4_reduced_identifiable_fit_completed",
        "method_evidence_id": method.evidence_id,
        "result": asdict(result),
        "v3_performance_comparison_report_only": comparison,
        "fit_attempted": True,
    ...
    summary = {
        ...
        "all_modeled_component_margins_inside_unit_interval": (
            result.all_modeled_component_margins_inside_unit_interval
        ),
        "other_margin_claimed_zero": result.other_margin_claimed_zero,
        "other_contribution_role": result.other_contribution_role,
