...
def _parser() -> argparse.ArgumentParser:
    description = (
        ...
        "issuer releases, untested OpenDART layouts, or qualitative commentary into "
        "fit evidence."
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    payload = {
        ...
        "company_profitability_certified_count": (
            audit.company_profitability_certified_count
        ),
        ...
        "q4_direct_quarter_derivation_allowed": (
            frontier.q4_direct_quarter_derivation_allowed
        ),
        "candidate_registration_enables_fit": (
            frontier.candidate_registration_enables_fit
        ),
        "candidate_registration_enables_holdout": (
            frontier.candidate_registration_enables_holdout
        ),
        "fit_enabled": audit.fit_enabled,
        "holdout_evaluation_enabled": audit.holdout_evaluation_enabled,
        "next_action": (
            "attempt_opendart_product_revenue_and_company_profitability_capture_"
            "then_separately_certify_four_field_cycle_drivers"
        ),
    }
