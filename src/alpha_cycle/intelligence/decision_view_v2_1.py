"""Preregistered internal Decision View and expectation-gap contracts for v2.1."""
...
class DecisionViewSelectionRuleSnapshot:
    """Selection rule frozen before candidate forecast values are registered."""
...
class DecisionViewSnapshot:
    """One internal view selected from a comparable forecast tournament."""
...
class DecisionExpectationGapSnapshot:
    """Provider/reference-preserving comparison with the frozen internal view."""
...
def build_decision_view(
    ...
    *,
    ...
    """Resolve a preregistered identity only after tournament comparability passes."""
    ...
    if rule.registered_at.date() > evaluation_date:
        raise ValueError("Decision View rule cannot be registered after evaluation_date")
...
def build_decision_expectation_gap(
    ...
    *,
    ...
    """Compare our view separately with each certified external reference."""
...
def _consensus_gap(
    ...
    consensus_value = float(observation.value)
...
def _price_gap(
    ...
    if observation.implied_value_krw is None:
        raise ValueError("available price-implied observation requires a numeric value")
    implied_value = float(observation.implied_value_krw)
    absolute_gap = decision_value_krw - implied_value
    return PriceImpliedGapObservation(
        reference_id=observation.reference_id,
        reference_kind=observation.reference_kind.value,
        reference_multiple=float(observation.reference_multiple),
