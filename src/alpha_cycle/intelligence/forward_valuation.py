"""Certified forward valuation state for Alpha Cycle Lab Decision System v2.1.

Forward valuation consumes only certified expectation observations and a complete point-in-time
market-cap state. It never substitutes trailing actual earnings for a missing forward estimate,
never silently aggregates providers, and never produces fair value or target prices.
"""
...
FORWARD_VALUATION_SCHEMA_VERSION = 2
...
class ForwardValuationObservation:
    """One transparent multiple tied to one certified forward expectation."""
    ...
    def __post_init__(self) -> None:
        ...
        _require_text(self.expectation_unit, "expectation_unit")
        _validate_sha(
            self.expectation_source_evidence_id,
            "expectation_source_evidence_id",
        )
        _require_aware(self.expectation_observed_at, "expectation_observed_at")
        ...
        if self.status is ForwardValuationStatus.AVAILABLE:
            if self.market_cap_krw is None:
                raise ValueError("available forward valuation requires market_cap_krw")
            if self.valuation_metric is None or self.multiple is None:
                raise ValueError("available forward valuation requires metric and multiple")
    ...
    def payload(self) -> dict[str, object]:
        return {
            ...
            "valuation_metric": (
                self.valuation_metric.value if self.valuation_metric is not None else None
            ),
...
class ForwardValuationStateSnapshot:
    ...
    def __post_init__(self) -> None:
        ...
        _validate_sha(
            self.expectation_state_snapshot_id,
        ...
        if len(set(keys)) != len(keys):
            raise ValueError(
                "ForwardValuationStateSnapshot contains duplicate observation keys"
            )
...
def build_forward_valuation_state(
    ...
    *,
    ...
    if not active.forward_valuation_revalidation_required_after_guardrail_merge:
        raise ValueError("active v2.1 policy does not require forward-valuation revalidation")
    if active.frozen_skhynix_2026q3_round_changed:
        raise ValueError("forward valuation cannot run under a guardrail that changes SK hynix Q3")
    ...
    _require_aware(capture, "forward valuation captured_at")
    ...
    if any(
        row.status is ForwardValuationStatus.UNSUPPORTED_EXPECTATION_METRIC
        for row in rows
    ):
        warnings.append(
    ...
    if directory.exists():
        ...
        try:
            ...
            manifest = {
                ...
                "expectation_state_snapshot_id": (
                    snapshot.expectation_state_snapshot_id
                ),
...
def _market_cap_lookup(frame: pd.DataFrame) -> dict[str, float | None]:
    ...
    for raw in frame.loc[:, columns].to_dict(orient="records"):
        ...
        complete_raw = raw["market_cap_complete"]
        if not isinstance(complete_raw, bool):
            raise ValueError(
                f"market_cap_complete must be a boolean for ticker: {ticker}"
            )
        ...
        output[ticker] = market_cap if complete_raw else None
    return output
