...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Run the outcome-blind legacy-taxonomy repair of the SK hynix exact-20-period "
            "historical target join and frozen chronological backtest."
        )
    )
    parser.add_argument("--evaluation-date", required=True, type=date.fromisoformat)
    parser.add_argument(
        "--execution",
        default=str(DEFAULT_COMPANY_GP_EX_ANTE_HISTORICAL_EXECUTION_V2),
    )
    parser.add_argument("--scope", default=str(DEFAULT_COMPANY_GP_EX_ANTE_SCOPE_FREEZE))
    parser.add_argument(
        "--bundle",
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    repair = load_frozen_historical_schema_repair_v2(Path(args.execution))
    join, result, capture, capture_reused, join_reused = (
        run_schema_repaired_historical_evaluation_v2(
            ...
            "folds": [
                {
                    ...
                    "residual_degrees_of_freedom": fold.residual_degrees_of_freedom,
    ...
    payload = {
        ...
        "schema_repair_status": repair.status,
        "schema_repair_version": repair.execution_version,
        "schema_repair_evidence_id": repair.evidence_id,
        "prior_execution_evidence_id": repair.prior_execution_evidence_id,
        "prior_v1_raw_payload_retrieval_disclosed": repair.raw_payloads_retrieved_in_v1,
        "prior_v1_target_observation_constructed": (
            repair.target_observation_constructed_in_v1
        ),
        "outcome_value_inspection_used_for_repair": (
            repair.outcome_value_inspection_used_for_repair
        ),
        "raw_target_capture_status": capture.status,
        "raw_target_capture_evidence_id": capture.evidence_id,
        "raw_target_capture_reused": capture_reused,
        "target_join_reused": join_reused,
