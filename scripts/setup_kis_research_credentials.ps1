...
$RequiredVariables = @("KIS_APP_KEY", "KIS_APP_SECRET")
...
if ($StatusOnly) {
    ...
    Write-Host "KIS research-only credentials configured; no account number is required."
    exit 0
}
...
Write-Host "KIS research-only credentials configured; account, holdings, balance, and order APIs remain disabled."
