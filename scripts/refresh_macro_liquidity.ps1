...
$MacroOutput = Join-Path $OutputRoot "macro-liquidity-evidence"
$Timeout = Get-OptionValue -Arguments $PipelineArguments -Name "--timeout-seconds"
if ([string]::IsNullOrWhiteSpace($Timeout)) {
    $Timeout = "15"
}

Write-Host "Refreshing official macro/liquidity evidence for $EvaluationDate (best effort)."
& $ProjectPython -m alpha_cycle.macro_liquidity_cli `
...
if ($exitCode -ne 0) {
    [Console]::Error.WriteLine(
        "Macro/liquidity refresh failed with exit code $exitCode; live pipeline will continue without treating stale macro evidence as current."
    )
}
exit 0
